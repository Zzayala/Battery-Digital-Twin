...
def _simular_fijo_numba(
    ...
    """
    Simulación de modo fijo acelerada con Numba.
    Implementa el mismo modelo físico que la versión Python (Dinámica + Térmica).
    """
    ...
    for vuelta in range(n_vueltas):
        ...
        for i in range(n_puntos_vuelta):
            ...
            v_cell_ocv = 0.0
            for c in ocv_coeffs:
                v_cell_ocv = v_cell_ocv * soc_curr + c
            v_ocv_pack = v_cell_ocv * n_s
            ...
            if soc_curr < 0: soc_curr = 0.0
            if soc_curr > 100: soc_curr = 100.0
...
def simular_modo_fijo(
    ...
    if NUMBA_AVAILABLE:
        try:
             # Preparar tipos para Numba
            t_full, soc_full, temps_full, umbral_full, P_elec_full, distancia_full, r_pack_full, soc_curr, distancia_acumulada = _simular_fijo_numba(
                P_elec_pack_vuelta.astype(np.float64),
                v_ms_vuelta.astype(np.float64),
                float(dt),
                int(n_vueltas),
                float(soc_max),
                float(cap_pack_as),
                ...
                float(mass_pack_kg),
                float(cp),
                f_ocv.coeffs.astype(np.float64),
                float(acc_umbral)
            )
    ...
    # === FALLBACK PYTHON (Lento) ===
    return _simular_modo_fijo_python(
        P_elec_pack_vuelta, v_ms_vuelta, dt, n_vueltas,
        soc_max, cap_celda, n_s, n_p, r_interna_mohm,
        temp_amb, refrigeracion, peso_celda_g, f_ocv, acc_umbral
    )


def _simular_modo_fijo_python(
    P_elec_pack_vuelta, v_ms_vuelta, dt, n_vueltas,
    soc_max,
    cap_celda, n_s, n_p,
    r_interna_mohm,
    ...
    """Versión Legacy Python de simulación fija."""
