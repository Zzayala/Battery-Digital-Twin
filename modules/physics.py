...
@jit(nopython=True)
def _calc_ocv_high_fidelity(soc, scaling_params):
    """
    Calcula OCV con polinomio de grado 9 y escalado dinámico.
    Replicando exactamente la lógica de obtener_funcion_ocv_polinomica.
    scaling_params = (v_base_0, rango_base, v_min, rango_user)
    """
    # Polinomio base (aprox 2.75V a 4.2V)
    # Clamp SOC 0-100 como en la original
    if soc < 0.0: soc_c = 0.0
    elif soc > 100.0: soc_c = 100.0
    else: soc_c = soc
    ...
    # Evaluacion (Direct power matching python implementation)
    v_raw = (c9 * soc_c**9
           - c8 * soc_c**8
           + c7 * soc_c**7
           - c6 * soc_c**6
    ...
    v_base_0 = scaling_params[0]
    rango_base = scaling_params[1]
    v_min = scaling_params[2]
    rango_user = scaling_params[3]
...
def _simular_fijo_numba(
    ...
    """
    Versión acelerada con Numba de simular_modo_fijo.
    """
    # Arrays de salida
    n_puntos_vuelta = len(P_elec_pack_vuelta)
    n_puntos_total = n_puntos_vuelta * n_vueltas
    ...
    return t_full, soc_full, temps_full, umbral_full, P_elec_full, distancia_full, r_pack_full, soc_curr, distancia_acumulada
...
def obtener_funcion_ocv_polinomica(v_min, v_nom, v_max):
    ...
    # PARAMETROS DE ESCALADO (PARA REPRODUCIR EN NUMBA)
    # Tuple: (v_base_0, rango_base, v_min, rango_user)
    f_ocv_hibrida.scaling_params = (v_base_0, rango_base, v_min, rango_user)
...
def simular_modo_fijo(
    ...
    # --- OPTIMIZACIÓN NUMBA ---
    if NUMBA_AVAILABLE and hasattr(f_ocv, 'scaling_params'):
        try:
            scaling_params = f_ocv.scaling_params

            # Ensure float types
            dt_val = float(dt)
            n_vueltas_val = int(n_vueltas)
            soc_max_val = float(soc_max)
            cap_pack_as_val = float(cap_pack_as)
            n_s_val = float(n_s)
            n_p_val = float(n_p)
            r_op_val = float(r_operative_cell_base)
            temp_amb_val = float(temp_amb)
            refrigeracion_val = float(refrigeracion)
            mass_pack_kg_val = float(mass_pack_kg)
            cp_val = float(cp)
