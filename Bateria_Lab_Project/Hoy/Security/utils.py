...
def mostrar_kpi_html(label, valor, unidad="", color_borde="#007acc"):
    ...
    safe_valor = html.escape(str(valor))
    safe_unidad = html.escape(str(unidad))
    safe_color = html.escape(str(color_borde))
    ...
    <div style="
        ...
        margin-bottom: 10px;">
        <p style="margin:0; color: #aaa; font-size: 11px; text-transform: uppercase;">{safe_label}</p>
        <p style="margin:0; font-size: 20px; font-weight: bold; color: white;">
            {safe_valor} <span style="font-size: 12px; color: #ccc;">{safe_unidad}</span>
        </p>
    </div>
    """
