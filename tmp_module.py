...
ax_graph1 = plt.subplot(4, 1, 1) 
ax_term = plt.subplot(4, 1, 2)
ax_soc = plt.subplot(4, 1, 3)
ax_res = plt.subplot(4, 1, 4)
...
def run_simulation(val=None):
    ...
    FACTOR_AJUSTE_POUCH = 1.5  # Penalización celdas pouch comerciales vs laboratorio
    FACTOR_R_EODV = 0.05       # Fracción del EODV atribuible a crecimiento real de R
                                # La métrica EODV incluye caída por pérdida de capacidad
                                # (~95%) y crecimiento real de impedancia (~5%).
                                # Calibrado para R(EOL) ≈ ×1.75 de R_nominal.

    # --- PRE-CÁLCULO: C-rate y consulta IA (necesario para envejecimiento R) ---
    I_cell_pre = I_base_pack / n_p
    rms_current_pre = np.sqrt(np.mean(I_cell_pre**2))
    c_rate_efectivo = rms_current_pre / cap_celda if cap_celda > 0 else 0

    # Consulta a la IA para obtener tasas base de degradación
    try:
        tasa_pct_base_100, metric_res_norm_ia, info_ia = cerebro_ia.predecir_degradacion(c_rate_efectivo)
    except (TypeError, ValueError):
        tasa_pct_base_100, metric_res_norm_ia, info_ia = 0.00015, 0.00020, "Modo Fallback"
    ...
    if abs(cap_celda - 10.0) > 0.1:
         ...
         mass = cap_celda * 0.0205

    # Envejecimiento de R basado en IA (sincronizado con simulador de tracción)
    if factor_salud < 1.0 and tasa_pct_base_100 is not None and tasa_pct_base_100 > 1e-15:
        # Calcular degradación por ciclo en condiciones actuales (sin Arrhenius, pre-simulación)
        dod_pre = max((soc_max - soc_min) / 100.0, 0.01)
        soc_medio_pre = (soc_max + soc_min) / 2.0
        f_fatiga_pre = dod_pre ** 1.6
        f_stress_pre = 1.0 + 0.0027 * (abs(soc_medio_pre - 50.0) ** 2)
        deg_cap_ciclo_pre = tasa_pct_base_100 * cap_celda * FACTOR_AJUSTE_POUCH * f_fatiga_pre * f_stress_pre

        if deg_cap_ciclo_pre > 1e-15:
            # Ciclos equivalentes para alcanzar el SOH actual
            capacidad_perdida_actual = cap_celda * (1.0 - factor_salud)
            ciclos_equivalentes = capacidad_perdida_actual / deg_cap_ciclo_pre
            # Incremento de R basado en métrica normalizada de la IA (corregida por FACTOR_R_EODV)
            delta_R = (metric_res_norm_ia * FACTOR_R_EODV / cap_celda) * ciclos_equivalentes
            r_base_envejecida = r_nom_cell + delta_R
        else:
            r_base_envejecida = r_nom_cell
    else:
        r_base_envejecida = r_nom_cell  # SOH=100% o IA no disponible
    ...
    # --- CAMBIO A FÍSICA DE PACK COMPLETO ---
    n_cells_total = n_s * n_p
    mass_pack = mass * n_cells_total  # Masa total del bloque
    
    # Inversa de Capacidad Calorífica Total del Pack (m_total * cp)
    inv_mass_cp_pack = 1.0 / (mass_pack * cp)
    dt_inv_mass_cp_pack = dt * inv_mass_cp_pack
    inv_298 = 1.0 / 298.15
    ...
    for k in range(num_steps):
        ...
        # R Dinámica Final del paso (Celda)
        r_dyn = R_partial[k] * factor_arrhenius
        
        # Cálculo de Calor TOTAL del PACK
        # Q_gen_total = (Calor por celda) * (Num Celdas)
        q_gen_cell = I_sq_cell[k] * r_dyn
        Q_gen_pack = q_gen_cell * n_cells_total
        
        # Q_out_pack = Refrigeración Global (Caja) * DeltaT
        Q_out_pack = refrigeracion * (temp_curr - temp_amb)
        
        # Balance de Energía Pack
        dT = (Q_gen_pack - Q_out_pack) * dt_inv_mass_cp_pack
        temp_curr += dT
        temps[k] = temp_curr

    # 3.4. Cálculo Final Voltajes & Resistencias Vectorizado
    ...
    r_operative_vec = R_partial * factor_arr_vec # Resistencia Celda Dinámica (Vector)
    r_pack_total_vec = (r_operative_vec * n_s / n_p) + R_CONN_PACK # Resistencia Pack Dinámica (Vector)
    ...
    # 6. Degradación (HÍBRIDO CALIBRADO: IA + FÍSICA + ARRHENIUS)
    # -------------------------------------------------------------------------
    # Modelo sincronizado con el simulador de tracción (60V).
    # Base IA (pre-calculada) + Factores: Fatiga, Estrés SOC, Arrhenius.
    # -------------------------------------------------------------------------

    # A) Temperatura media (solo disponible tras la simulación)
    ...
    # C) Tasas base de IA (ya consultadas antes de la simulación)
    # tasa_pct_base_100 y metric_res_norm_ia ya disponibles

    # D) FACTORES FÍSICOS DE CORRECCIÓN

    # 1. Fatiga Mecánica (DoD) - Ley de Wöhler
    factor_fatiga = dod_decimal ** 1.6

    # 2. Estrés por Zona de SOC (Curva en V centrada en 50%)
    distancia_al_centro = abs(soc_promedio - 50.0)
    factor_stress_soc = 1.0 + (0.0027 * (distancia_al_centro ** 2))

    # 3. Factor Arrhenius — Estrés Térmico (sincronizado con simulador 60V)
    factor_arrhenius_deg = 2.0 ** ((temp_promedio - 25.0) / 10.0)

    # E) CÁLCULO FINAL CON CALIBRACIÓN
    if tasa_pct_base_100 is not None:
        # Fórmula: Base_IA * Pouch * Fatiga * Estrés_SOC * Arrhenius
        deg_por_ciclo = (tasa_pct_base_100 * cap_celda) * FACTOR_AJUSTE_POUCH * factor_fatiga * factor_stress_soc * factor_arrhenius_deg

        metodo_usado = (f"Híbrido (DoD {dod_slider_pct}% | "
                       f"Arr. x{factor_arrhenius_deg:.2f} | "
                       f"Tech x{FACTOR_AJUSTE_POUCH})")
    else:
        deg_por_ciclo = 0.0
    ...
    # GRÁFICA 4: RESISTENCIA
    ax_res.clear()
    ax_res.xaxis.set_major_formatter(fmt)
    # Convertir a miliOhms para visualización clara
    r_pack_mohm = r_pack_total_vec * 1000.0
    r_cell_mohm = r_operative_vec * 1000.0
    
    ax_res.plot(t_base, r_pack_mohm, color='#1f77b4', label='Res. Pack (mΩ)')
    ax_res.plot(t_base, r_cell_mohm, color='#ff7f0e', linestyle='--', linewidth=1.5, label='Res. Celda (mΩ)')
    
    ax_res.set_ylabel("Resistencia (mΩ)")
    ax_res.set_title("DINÁMICA DE RESISTENCIA INTERNA")
    ax_res.legend(loc='best', fontsize='x-small', framealpha=0.5)
...
ax_radio = plt.axes([0.05, 0.75, 0.15, 0.15], facecolor='#f0f0f0')
...
plt.text(0.05, 0.93, "1. CONFIGURACIÓN RÁPIDA", transform=fig.transFigure, fontsize=11, fontweight='bold', color='blue')

ax_radio_plot = plt.axes([0.25, 0.75, 0.15, 0.15], facecolor='#e6e6e6')
...
plt.text(0.25, 0.93, "VISUALIZACIÓN SUPERIOR", transform=fig.transFigure, fontsize=11, fontweight='bold')
